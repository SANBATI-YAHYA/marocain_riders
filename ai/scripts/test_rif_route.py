"""
End-to-end recommendation test for a rider profile targeting the
Northern / Rif Mountains route (RT006: Rif Mountains Loop).

Profile:
  - Intermediate rider, adventure bike
  - Loves scenic mountain roads, culture, hiking vibes
  - 4-day trip budget ~EUR50/day
  - Travelling in May (peak Rif season)

Expected result:
  RT006 (Rif Mountains Loop) should be the top or a top-3 route.
  Day plan should include Fès (PL012) and Chefchaouen (PL013).
  Stops should reference ST014/ST015, RS010/RS011, GS010/GS011.

Usage:
    python scripts/test_rif_route.py
"""
...
print("=" * 60)
print("  MOROCCO MOTO RECOMMENDER - Rif Mountains Test")
print("=" * 60)
...
# ══════════════════════════════════════════════════
#  Rif-Oriented Rider Profile
# ══════════════════════════════════════════════════
print("\n[3/5] Building Rif-focused rider profile...")

rif_profile = UserProfile(
    ...
    sleep_preference="riad/guesthouse",
    daily_ride_km_tolerance=160,
    travel_month="May",
...
print(
    f"""
  Rider Profile (Rif Target):
  Bike: adventure
  Experience: {rif_profile.rider_experience}
  Budget: EUR{rif_profile.budget_per_day_eur}/day
  Duration: {rif_profile.trip_duration_days} days
  Vibes: {', '.join(rif_profile.preferred_vibes)}
  Sleep: {rif_profile.sleep_preference}
  Daily km: {rif_profile.daily_ride_km_tolerance} km max
  Travel month: {rif_profile.travel_month}
"""
)
...
# ══════════════════════════════════════════════════
print("[3/5] Scoring all routes for Rif profile...\n")
...
for route in all_routes:
    total, breakdown = score_route(route, all_places, rif_profile)
...
for i, (r, score, bd) in enumerate(scored, 1):
    ...
    print(
        f"       Diff: {bd['difficulty']:.2f}  Bike: {bd['bike_suit']:.2f}  "
        f"Vibe: {bd['vibe']:.2f}  Dur: {bd['duration']:.2f}  "
        f"Fuel: {bd['fuel_safety']:.2f}  Season: {bd['season']:.2f}"
    )

print(f"\n  RT006 Rank: #{rt006_rank}")
if rt006_rank and rt006_rank <= 3:
    print("  PASS: RT006 is in the top 3 routes!")
else:
    print(f"  WARNING: RT006 ranked #{rt006_rank} — may need profile tuning")
...
request = RecommendationRequest(
    user_profile=rif_profile,
    additional_preferences=(
        "I want to explore the Rif Mountains and Chefchaouen. "
        "Scenic mountain roads with cultural stops. "
        "Starting from Fès."
    ),
)
...
# ══════════════════════════════════════════════════
print(f"\n{'='*60}")
print("  RECOMMENDATION RESULTS - RIF ROUTE TEST")
print(f"{'='*60}")
...
if response.recommended_route:
    ...
    # Validation: check if Rif-related places are present
    is_rif = rr.route_id == "RT006"
    print(f"\n  Route is RT006 (Rif Mountains Loop): {is_rif}")

if response.alternative_routes:
    print(f"\n  Alternative Routes:")
    for alt in response.alternative_routes:
        print(
            f"    - {alt.route_name} (score: {alt.total_score:.3f}, {alt.distance_km}km)"
        )
...
if response.day_plan:
    print(f"\n  Day-by-Day Plan:")
    fes_found = False
    chefchaouen_found = False
    for day in response.day_plan:
        print(
            f"    Day {day.day_number}: {day.start_place} -> {day.end_place} ({day.distance_km}km)"
        )
        for stop in day.stops:
            print(
                f"      [{stop.stop_type.upper()}] {stop.name} -- {stop.notes or ''}"
            )
        if "F" in day.start_place or "F" in day.end_place:
            fes_found = True
        if "Chef" in day.start_place or "Chef" in day.end_place:
            chefchaouen_found = True
    print(f"\n  Fès in day plan: {fes_found}")
    print(f"  Chefchaouen in day plan: {chefchaouen_found}")
else:
    print("\n  Day plan: (none built)")
...
if response.stop_points:
    ...
    for sp in response.stop_points:
        coords = (
            f"({sp.latitude:.3f}, {sp.longitude:.3f})" if sp.latitude else "(no coords)"
        )
        print(
            f"    [{sp.stop_type.upper():7s}] {sp.name:<30s} day {sp.estimated_day}  "
...
if response.retrieved_context_summary:
    print(
        f"\n  Semantic Context ({len(response.retrieved_context_summary)} chunks):"
    )
...
# Check 1: RT006 in top 3
checks.append(("RT006 in top 3 scoring", rt006_rank is not None and rt006_rank <= 3))

# Check 2: Recommended route is RT006
rec_is_rt006 = (
    response.recommended_route and response.recommended_route.route_id == "RT006"
)
checks.append(("RT006 selected as recommendation", rec_is_rt006))

# Check 3: Fès longitude is negative (west of Greenwich)
fes_place = next((p for p in all_places if p.place_id == "PL012"), None)
fes_lon_ok = fes_place and fes_place.longitude is not None and fes_place.longitude < 0
checks.append(("PL012 Fès longitude is negative", fes_lon_ok))

# Check 4: Chefchaouen exists
chef_place = next((p for p in all_places if p.place_id == "PL013"), None)
checks.append(("PL013 Chefchaouen is in DB", chef_place is not None))

# Check 5: RT006 exists
rt006_route = next((r for r in all_routes if r.route_id == "RT006"), None)
checks.append(("RT006 Rif Mountains Loop in DB", rt006_route is not None))

# Check 6: Some stops were planned
has_stops = bool(response.stop_points) or bool(response.fuel_stops)
checks.append(("Stop planning produced stops", has_stops))
...
output_path = PROJECT_ROOT / "test_output_rif.json"
